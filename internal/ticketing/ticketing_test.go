...
func TestCreateTicketOptionParamsValidate(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			params:  ticketing.CreateTicketOptionParams{Name: "GA", Allocation: 0, BucketCount: 0},
			wantErr: false,
		},
		{
			name:    "zero allocation rejects bucket_count",
...
func TestResolveBucketCount(t *testing.T) {
	...
	if got := ticketing.ResolveBucketCount(100, nil); got != 1 {
		t.Fatalf("omitted = %d, want 1", got)
	}
	n := 8
	if got := ticketing.ResolveBucketCount(100, &n); got != 8 {
		t.Fatalf("explicit = %d, want 8", got)
	}
	if got := ticketing.ResolveBucketCount(0, nil); got != 0 {
		t.Fatalf("zero allocation omitted = %d, want 0", got)
	}
}
...
func TestSplitAllocation(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "zero", total: 0, bucketCount: 0, want: nil},
